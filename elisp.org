...
** 获取buffer 信息
    ...
    (get-buffer BUFFER-OR-NAME) : 返回指定buffer对象, 若不存在则返回
...
* 插入文本
  ...
  (insert-buffer-substring BUFFER-OR-NAME &optional START END): 将指定
  buffer中, START和END之间的内容,拷贝到当前buffer光标后.若不指定起始位
  置或终止位置则拷贝源buffer中的全部内容.

  (insert-buffer BUFFER-OR-NAME) : 将指定buffer中的所有内容插入到当前
  光标后. 等价于: (insert-buffer-substring BUFFER-OR-NAME).

  (copy-to-buffer BUFFER-OR-NAME START END): 将当前buffer中位于[start,
  , end)之间的文本拷贝到指定buffer中(会清除该buffer中原有内容).

  (append-to-buffer BUFFER-OR-NAME START END): 将当前buffer中位于
